...
class UserAuth extends Model {
    ...
    public function loggedin() {
        ...
        if (isset($_GET['c']) && isset($_SESSION['auth'])) {
            ...
            if ($GLOBALS['config']['acl'][$controller][$access] == true) {
                return true;
            } else {
                return false;
            }
    ...
    public function writeSession() {
        ...
        $sql = "SELECT avatar FROM CS_Avatars WHERE userid = :userid;";
        try {
            $rs = NULL;
            $rs = $this->DBH->prepare($sql);
            $rs->execute(array(':userid' => $this->values[0]));
        } catch (PDOException $e) {
